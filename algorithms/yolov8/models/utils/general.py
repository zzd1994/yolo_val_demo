...
def process_mask_native(protos, masks_in, bboxes, shape, scale_fill=False):
    """
    Crop after upsample.
...
#         # t2_time = time.time()
#         # print('xi', type(xi), xi)
#         # print('xc', xc[xi], xc[xi].shape, xc.dtype)
#         # print('xxx', x.dtype, x.shape, x.device)
#
#         x = x[xc[xi]]  # confidence
#         # print(f'nms2:{round((time.time() - t2_time) * 1000, 2)}ms')
#         # Cat apriori labels if autolabelling
#
#         if labels and len(labels[xi]):
#             lb = labels[xi]
...
#         t1_time = time.time()
#         i = torchvision.ops.nms(boxes, scores, iou_thres)  # NMS
#         # print(f'nms{round((time.time() - t1_time) * 1000, 2)}ms')
...
#             break  # time limit exceeded
#     # print(f'nms1:{round((time.time() - t) * 1000, 2)}ms')
#     return output
