...
import torch
# ----------------------------------------
from .models.utils.general import non_max_suppression, scale_coords, scale_boxes
from .models.common import DetectMultiBackend
from .models.utils.general import process_mask, process_mask_native
# --------------------------------------------------

class YOLOv8Segment(object):
    ...
    def __init__(self, target_size, padding_color, conf_thres, iou_thres, device, net_conf, weight_path, classes,
        ...
        self._model = DetectMultiBackend(weight_path, yaml=net_conf, nc=len(classes), device=self._device,
                                         data=classes, fp16=True, segment=True)
        # print("after _model", self._model)
    ...
    def seg(self, image_list: list, *args, **kwargs):
        # s1_time = time.time()
        ...
        # ----------------------------------------
        pred = self._model(batch, augment=False)
        batch_output = pred[0]
        batch_proto = pred[1][-1]
        # print('batch_output', batch_output.shape)
        # print('batch_proto', batch_proto.shape)
        # batch_other = batch_other.detach().cpu()
        # print('batch_other', batch_other)
        # s3_time = time.time()
        # print(f'模型{round((s3_time - s2_time) * 1000, 2)}ms')
        batch_pred = non_max_suppression(
            batch_output, nc=len(self._classes),
        ...
        # ----------------------------------------
        return self._postprocess(batch_pred, batch.shape[-2:], batch_image_size, batch_proto)
    ...
    def _postprocess(self, batch_pred, input_image_shape, batch_image_size, batch_proto):
        results = []

        for idx, (pred_item, image_size, proto_item) in enumerate(zip(batch_pred, batch_image_size, batch_proto)):
            result_item = []
            if (pred_item is not None) and len(pred_item):
                # pred_item[:, :4] = scale_coords(
                #     input_image_shape, pred_item[:, :4], image_size).round()
                if self._scaleFill:
                    x_ratio = input_image_shape[1]/image_size[1]
                    y_ratio = input_image_shape[0]/image_size[0]
                    gain = [torch.tensor([x_ratio, y_ratio, x_ratio, y_ratio]).to(self._device)]
                    pad = [0, 0]
                    ratio_pad = (gain, pad)
                else:
                    ratio_pad = None
                pred_item[:, :4] = scale_boxes(
                    input_image_shape, pred_item[:, :4], image_size, ratio_pad=ratio_pad).round()
                masks_item = process_mask_native(proto_item, pred_item[:, 6:], pred_item[:, :4], image_size,
                                                 scale_fill=self._scaleFill)
                masks_item = masks_item.int().cpu().numpy().astype('uint8')
                for i, (*p1p2, conf, klass_idx) in enumerate(pred_item[:, :6]):
                    ...
                    result_item.append((klass_name, score, (x1, y1, x2, y2), (xo, yo, w, h), masks_item[i]))
